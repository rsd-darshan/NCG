"""
NCG plotting utilities.

Packaged copy of plotting helpers so CLI entrypoints work after installation.
"""
...
def plot_accuracy_over_tasks(
    ...
    ax.legend()
    ax.set_xticks(list(range(t_count)) if t_count else [])
    ax.set_ylim(0, 1.05)
    fig.tight_layout()
...
def plot_forgetting_curve(
    ...
    ax.set_yticks(range(t_count))
    ax.set_xticklabels(range(n_cols))
    ax.set_yticklabels(range(t_count))
...
def plot_meta_parameters(
    ...
    axes[0].set_title("Exploration (alpha)")
    axes[0].set_ylim(0, 1.05)
    axes[1].plot(x, beta_per_epoch, color="tab:green")
    axes[1].set_ylabel("beta")
    axes[1].set_title("Complexity penalty (beta)")
